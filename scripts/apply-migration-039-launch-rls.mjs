#!/usr/bin/env node
/** Aplica migración 039 — chat + reviews RLS. npm run db:apply-launch-rls */
...
const MIGRATION = '039_launch_chat_reviews_rls.sql';
...
try {
  if (hasAccessToken) {
    ...
    if (!ok) throw new Error('SUPABASE_ACCESS_TOKEN inválido o ausente');
  } else {
    try {
