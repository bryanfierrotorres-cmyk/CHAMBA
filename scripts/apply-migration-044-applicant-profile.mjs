#!/usr/bin/env node
/** Aplica migración 044 — bio + distancia en postulaciones. npm run db:apply-applicant-profile */
...
const MIGRATION = '044_applicant_profile_bio_distance.sql';
    ...
    SELECT
      EXISTS (
        ...
        WHERE table_schema = 'public'
          ...
          AND column_name = 'applicant_lat'
      ) AS has_applicant_lat,
      EXISTS (
        SELECT 1 FROM pg_proc p
        JOIN pg_namespace n ON n.oid = p.pronamespace
        WHERE n.nspname = 'public'
          AND p.proname = 'fn_haversine_km'
      ) AS has_haversine
  `;
  if (dbOrNull) {
...
try {
  ...
  const okLat = check?.has_applicant_lat === true || check?.has_applicant_lat === 't';
  const okHaversine = check?.has_haversine === true || check?.has_haversine === 't';
  if (!okLat || !okHaversine) {
    throw new Error(`Verificación falló: ${JSON.stringify(check)}`);
  }
  console.log('✅ Migración 044 aplicada — applicant_lat + fn_haversine_km + RPC enriquecido');
} catch (err) {
