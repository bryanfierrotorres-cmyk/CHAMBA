#!/usr/bin/env node
/** Aplica migración 051 — completed_at al avanzar a completed. npm run db:apply-worker-advance-complete */
import { readFileSync, existsSync } from 'fs';
import { join, dirname } from 'path';
import { fileURLToPath } from 'url';

const ROOT = join(dirname(fileURLToPath(import.meta.url)), '..');
const SQL_PATH = join(ROOT, 'supabase', 'migrations', '051_worker_advance_complete_assignment.sql');
...
async function main() {
  if (dbUrl) {
    ...
    try {
      ...
      await db.query(sql);
      await db.end();
      console.log('✅ Migración 051 aplicada (Postgres directo)');
      return;
    } catch (err) {
  ...
  console.log('✅ Migración 051 aplicada (Management API)');
}
