...
const SQL_PATH = join(ROOT, 'supabase', 'migrations', '062_technician_trust_ecosystem.sql');
...
async function main() {
  if (dbUrl) {
    ...
    try {
      await db.connect();
      await db.query(sql);
      console.log('✅ Migración 062 aplicada (Postgres directo)');
      return;
    } catch (err) {
  ...
  console.log('✅ Migración 062 aplicada (Management API)');
}
