...
const SQL_PATH = join(ROOT, 'supabase', 'migrations', '063_seed_trust_ecosystem.sql');
...
async function main() {
  if (dbUrl) {
    ...
    try {
      await db.connect();
      await db.query(sql);
      console.log('✅ Migración 063 aplicada (Postgres directo)');
      return;
    } catch (err) {
  ...
  console.log('✅ Migración 063 aplicada (Management API)');
}
