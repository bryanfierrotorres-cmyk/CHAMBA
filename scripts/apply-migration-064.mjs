...
const SQL_PATH = join(ROOT, 'supabase', 'migrations', '064_analytics_events.sql');
...
async function main() {
  if (dbUrl) {
    ...
    try {
      await db.connect();
      await db.query(sql);
      console.log('✅ Migración 064 aplicada (Postgres directo)');
      return;
    } catch (err) {
  ...
  console.log('✅ Migración 064 aplicada (Management API)');
}
