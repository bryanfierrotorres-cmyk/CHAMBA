...
const SQL_PATH = join(ROOT, 'supabase', 'migrations', '065_bidding_flow.sql');
...
async function main() {
  if (dbUrl) {
    ...
    try {
      await db.connect();
      await db.query(sql);
      console.log('✅ Migración 065 aplicada (Postgres directo)');
      return;
    } catch (err) {
  ...
  console.log('✅ Migración 065 aplicada (Management API)');
}
