-- CHAMBA 020 — Límites: cliente máx. 2 solicitudes activas; técnico máx. 2 chambas activas
...
AS $$
  ...
  WHERE ja.worker_id = p_worker_id
    ...
    AND ja.selection_status IN ('pending', 'approved');
$$;

GRANT EXECUTE ON FUNCTION count_client_active_jobs(UUID) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION count_worker_active_commitments(UUID) TO anon, authenticated;

-- ── create_client_job: máx. 2 solicitudes activas por cliente ───────────────
...
-- ── accept_job: máx. 2 chambas activas por técnico ─────────────────────────
...
-- ── Al elegir técnico, validar cupo del técnico ────────────────────────────
...
-- Incluir selection_status en agenda del técnico
...
GRANT EXECUTE ON FUNCTION accept_job(UUID, UUID) TO anon, authenticated;
