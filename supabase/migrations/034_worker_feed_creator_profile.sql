-- CHAMBA 034 — Feed técnico incluye perfil del cliente (nombre + avatar)
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(entry ORDER BY sort_at DESC), '[]'::jsonb) INTO v_rows
  FROM (
    SELECT
      (
        to_jsonb(j)
