-- CHAMBA 038 — Aislamiento técnico: RLS en jobs/assignments + RPC acotados
-- Objetivo: un técnico solo lee jobs open (radar) o los suyos asignados/postulados.
-- Sin pilot_anon_assignments_select (USING true) ni get_worker_assignments abierto a anon.
...
-- ── Quitar políticas piloto permisivas (añadidas fuera de migraciones) ───────
DROP POLICY IF EXISTS "pilot_anon_jobs_select" ON jobs;
DROP POLICY IF EXISTS "pilot_anon_jobs_insert" ON jobs;
DROP POLICY IF EXISTS "pilot_anon_jobs_update" ON jobs;

-- Cliente: políticas canónicas (pueden faltar si solo existían pilot_anon_*)
...
-- ── jobs: reemplazar lectura amplia de técnicos ─────────────────────────────
DROP POLICY IF EXISTS "jobs: approved worker read" ON jobs;

DROP POLICY IF EXISTS "jobs: worker read scoped" ON jobs;
CREATE POLICY "jobs: worker read scoped"
  ON jobs FOR SELECT
  TO authenticated
  USING (
    EXISTS (
...
-- ── job_assignments: técnico solo las suyas ─────────────────────────────────
DROP POLICY IF EXISTS "assignments: worker select own" ON job_assignments;
CREATE POLICY "assignments: worker select own"
  ON job_assignments FOR SELECT
  TO authenticated
  USING (worker_id = auth.uid());

DROP POLICY IF EXISTS "assignments: admin all" ON job_assignments;
...
-- Quitar lectura anónima total (fuga de postulaciones de todos los técnicos)
DROP POLICY IF EXISTS "pilot_anon_assignments_select" ON job_assignments;

-- ── RPC: agenda del técnico — solo propio worker o admin ────────────────────
...
-- ── RPC: radar — solo jobs open (sin filtrar taken/in_progress ajenos) ──────
...
BEGIN
  ...
  -- Este RPC es exclusivamente el radar de solicitudes abiertas
...
-- get_open_jobs_feed (JWT): también acotar a open
