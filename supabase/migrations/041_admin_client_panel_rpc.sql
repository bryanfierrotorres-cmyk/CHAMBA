-- CHAMBA 041 — Paneles admin/cliente sin depender de JWT (login teléfono / piloto)
SET statement_timeout = '120s';

-- ── Admin: torre de control (jobs + asignaciones) ───────────────────────────
CREATE OR REPLACE FUNCTION get_admin_control_jobs(p_admin_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  RETURN COALESCE((
    SELECT jsonb_agg(job_row ORDER BY job_row->>'created_at' DESC)
    FROM (
          ...
          SELECT jsonb_agg(
            jsonb_build_object(
              ...
              'worker', (
                ...
                FROM profiles w WHERE w.id = ja.worker_id
              )
            )
          )
...
GRANT EXECUTE ON FUNCTION get_admin_control_jobs(UUID) TO anon, authenticated;

-- ── Cliente: mis solicitudes con técnico asignado ───────────────────────────
...
AS $$
  ...
  FROM (
    ...
    FROM jobs j
    LEFT JOIN profiles w ON w.id = j.assigned_worker_id
