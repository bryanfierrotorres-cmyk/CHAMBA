-- CHAMBA 045 — Última ubicación del técnico (Radar / Agenda) + fallback en distancia de postulaciones
-- NO modifica RLS de jobs, profiles ni auth por teléfono.
...
COMMENT ON COLUMN worker_profiles.last_lat IS
  'Última latitud GPS reportada por el técnico (Radar/Agenda)';
COMMENT ON COLUMN worker_profiles.last_lng IS
  'Última longitud GPS reportada por el técnico (Radar/Agenda)';

CREATE OR REPLACE FUNCTION update_worker_last_location(
  p_worker_id UUID,
...
BEGIN
  ...
  v_lat := CASE
    WHEN p_lat IS NOT NULL AND (ABS(p_lat) > 0.0001 OR ABS(COALESCE(p_lng, 0)) > 0.0001)
    THEN p_lat ELSE NULL END;
  v_lng := CASE
    WHEN p_lng IS NOT NULL AND (ABS(COALESCE(p_lat, 0)) > 0.0001 OR ABS(p_lng) > 0.0001)
    THEN p_lng ELSE NULL END;

  IF v_lat IS NULL OR v_lng IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error', 'Coordenadas inválidas');
  END IF;

  INSERT INTO worker_profiles (worker_id, skills, availability_status, last_lat, last_lng, last_location_at)
  VALUES (p_worker_id, '{}', 'offline', v_lat, v_lng, NOW())
...
GRANT EXECUTE ON FUNCTION update_worker_last_location(UUID, DOUBLE PRECISION, DOUBLE PRECISION)
  TO authenticated, anon;

-- Postulaciones: distancia con coords al postular O última ubicación del perfil técnico
...
BEGIN
  ...
  FROM (
    SELECT
      ...
      COALESCE(ja.applicant_lat, wp.last_lat) AS worker_lat,
      COALESCE(ja.applicant_lng, wp.last_lng) AS worker_lng,
      ROUND(
        fn_haversine_km(
          v_job.lat,
          v_job.lng,
          COALESCE(ja.applicant_lat, wp.last_lat),
          COALESCE(ja.applicant_lng, wp.last_lng)
        )::numeric,
        1
      ) AS distance_km
