-- CHAMBA 052 — Al completar chamba: acreditar billetera (payment_status) y métricas técnico
...
BEGIN
  ...
  v_status := CASE p_phase
    ...
    WHEN 'arrived' THEN 'in_progress'::job_status
    ELSE 'taken'::job_status
  END;
  ...
  IF p_phase = 'completed' THEN
    UPDATE job_assignments
       SET completed_at = v_now,
           payment_status = CASE
             WHEN payment_status::text IN ('paid', 'processing') THEN payment_status
    ...
    IF v_rows > 0 THEN
      INSERT INTO worker_profiles (worker_id, skills, availability_status, total_jobs_done)
      VALUES (p_worker_id, '{}', 'available', 1)
      ON CONFLICT (worker_id) DO UPDATE
        SET total_jobs_done = COALESCE(worker_profiles.total_jobs_done, 0) + 1;
    END IF;
  END IF;
