-- CHAMBA 053 — Chat cliente↔técnico con login teléfono (mismo canal que agenda 046)
-- Problema: job_chat_* exigía auth.uid() = created_by/assigned_worker_id exacto;
-- sin JWT o con perfil duplicado por teléfono, no se podía leer ni enviar mensajes.
...
CREATE OR REPLACE FUNCTION job_chat_user_matches(p_profile_id UUID, p_user_id UUID)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT p_profile_id IS NOT NULL
    ...
    AND (
      ...
      OR fn_profiles_same_phone(p_profile_id, p_user_id)
    );
$$;

CREATE OR REPLACE FUNCTION job_chat_actor_is(p_profile_id UUID)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT job_chat_user_matches(p_profile_id, auth.uid());
$$;
...
AS $$
  SELECT EXISTS (
    ...
    WHERE j.id = p_servicio_id
      ...
      AND (
        job_chat_actor_is(j.created_by)
        OR job_chat_actor_is(j.assigned_worker_id)
        OR EXISTS (
          SELECT 1
          FROM job_assignments ja
          WHERE ja.job_id = j.id
            AND job_chat_actor_is(ja.worker_id)
...
AS $$
  SELECT EXISTS (
    ...
    WHERE j.id = p_servicio_id
      AND j.status <> 'cancelled'
      AND (
        job_chat_user_matches(j.created_by, p_user_id)
        OR job_chat_user_matches(j.assigned_worker_id, p_user_id)
        OR EXISTS (
          SELECT 1
          FROM job_assignments ja
          WHERE ja.job_id = j.id
            AND job_chat_user_matches(ja.worker_id, p_user_id)
            AND ja.selection_status = 'approved'
        )
      )
  );
...
CREATE POLICY mensajes_insert_participant ON mensajes
  FOR INSERT
  WITH CHECK (
    job_chat_user_matches(remitente_id, auth.uid())
...
BEGIN
  ...
  IF v_uid IS NULL THEN
    ...
    END IF;
  ELSIF NOT job_chat_user_matches(p_remitente_id, v_uid) THEN
    RETURN jsonb_build_object('success', false, 'error', 'No autorizado');
  ELSIF NOT job_chat_user_can_write(p_servicio_id, p_remitente_id) THEN
    RETURN jsonb_build_object(
      'success', false,
...
BEGIN
  IF v_uid IS NULL THEN
    ...
    END IF;
  ELSIF NOT job_chat_user_matches(p_caller_id, v_uid) THEN
    RETURN jsonb_build_object('success', false, 'error', 'No autorizado');
  ELSIF NOT job_chat_user_can_access(p_servicio_id, p_caller_id) THEN
...
GRANT EXECUTE ON FUNCTION job_chat_user_can_access(UUID, UUID) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION job_chat_user_can_write(UUID, UUID) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION job_chat_user_matches(UUID, UUID) TO anon, authenticated;
